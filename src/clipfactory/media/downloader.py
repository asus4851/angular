...
def download_section(yt_video_id: str, start_sec: float, end_sec: float, dest_dir: Path) -> Path:
    ...
    stem = f"{yt_video_id}_{int(start_sec)}_{int(end_sec)}"
    ...
    padded_start = max(0.0, start_sec - _PADDING_SEC)
    padded_end = end_sec + _PADDING_SEC
...
def download_full(yt_video_id: str, dest_dir: Path) -> Path:
    """Download the whole video (useful for short videos where sectioning gains little)."""
    dest_dir.mkdir(parents=True, exist_ok=True)
    stem = f"{yt_video_id}_full"

    cached = _existing_output(dest_dir, stem)
    if cached is not None:
        logger.info("download_full: cache hit for %s -> %s", stem, cached)
        return cached
    ...
    try:
        ...
        raise DownloadError(f"Failed to download {yt_video_id!r}: {exc}") from exc
